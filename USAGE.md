...
## 五、v0.9.2 重要更新：PM工作分配複選功能

### PM工作分配規則與操作

版本 0.9.2 重寫了工作分配模式規則，新增PM工作分配複選功能，為護理師工作分配提供更大的靈活性。

#### PM工作分配特色
- **複選模式**：PM工作分配可與主要工作分配並存，不會互相衝突
- **週一到週五限定**：PM按鈕僅在週一到週五顯示，符合實際業務需求
- **循環選項**：PM按鈕支援 PMTAE → PMC → PMF2 → 取消 的循環選擇
- **智能衝突檢測**：同一天不能有兩個護理師選擇相同的PM工作分配

#### 操作說明

##### 對於護理長（編輯模式）
1. **進入編輯模式**：在週班表頁面點擊「編輯」按鈕
2. **選擇主要工作分配**：點擊護理師姓名下方的工作分配按鈕（如TAE、C、F2等）
3. **選擇PM工作分配**：點擊粉紅色的「PM」按鈕，循環選擇 PMTAE → PMC → PMF2 → 取消
4. **檢視複選結果**：選擇完成後，儲存格會顯示「主要工作/PM工作」格式（如「TAE/PMTAE」）
5. **儲存變更**：點擊「儲存」按鈕保存所有工作分配

##### 工作分配顯示格式
- **僅主要工作**：直接顯示工作分配代碼（如「TAE」）
- **主要工作+PM工作**：顯示為斜線分隔格式（如「TAE/PMTAE」）
- **僅PM工作**：顯示為「/PM工作」格式（如「/PMC」）

##### 衝突檢測機制
- **主要工作分配衝突**：同一天不能有兩個護理師選擇相同的主要工作分配
- **PM工作分配衝突**：同一天不能有兩個護理師選擇相同的PM工作分配
- **智能選項過濾**：當某個PM選項已被其他護理師選擇時，按鈕會自動跳過該選項

#### 適用對象
- **麻醉專科護理師**：可使用完整的工作分配功能，包括主要工作分配和PM工作分配
- **麻醉科Leader**：可使用完整的工作分配功能，包括主要工作分配和PM工作分配  
- **護理長**：可使用完整的工作分配功能，包括主要工作分配和PM工作分配
- **恢復室護理師**：僅使用主要工作分配功能，不顯示PM按鈕

#### 資料儲存與讀取
- **儲存格式**：系統將工作分配以「主要工作/PM工作」格式儲存在 area_code 欄位
- **自動解析**：開啟編輯模式時，系統會自動解析現有的工作分配並正確顯示按鈕狀態
- **向下相容**：舊格式的工作分配資料仍能正常顯示和編輯
