# 麻醉科護理班表管理系統使用指南 (v0.10.0 beta)

本文檔詳細說明麻醉科護理班表管理系統的各項功能和使用方法。

## 系統概覽

麻醉科護理班表管理系統是專為麻醉科部門設計的班表管理工具，支援護理長排班和護理師查詢班表的完整流程。系統支援角色權限管理、公式化排班、自動分配、調班管理等完整功能，並整合了醫師班表查詢系統。
...
- 可直接點擊「使用Passkey登入」，建議先輸入用戶名以縮小憑證列表
- 使用生物辨識（指紋、Face ID）或 PIN 驗證，支援跨裝置
