...
### 基礎分數計算
- **初始分數**: 0 分
- **加班正分**: 根據班別類型加分（A=2.0, B=1.0, C=0.8, D=0.3, E/F=0.0）
- **未加班白班扣分**: -0.365分/天（經過數學計算的平衡值）
...
### 範例計算（22個工作天，出勤率95%）
```
實際工作天數 = 22 × 0.95 = 20.9 ≈ 20 天
分配：3個A班 + 2個B班 + 1個C班 = 6天加班
加班分數 = (3×2.0) + (2×1.0) + (1×0.8) = 8.8 分
沒加班白班 = 20 - 6 = 14 天
白班負分 = 14 × (-0.365) = -5.11 分
最終分數 = 8.8 - 5.11 = 3.69 分
```
