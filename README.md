...
### 加班管理
- 自動識別月班表中的加班人員（A班）
- **自動加班分配系統**：統一分數導向算法，確保所有班別都按分數最低優先原則分配
- 加班人員排序標記（A-F）
- 加班記錄持久化存儲
- 權限控制（僅護理長和管理員可編輯）
- 批量更新和修改
...
### v0.10.0 更新摘要
- LINE 掃碼登入（桌機顯示 QR、手機授權、桌機輪詢自動登入）
- LINE 頭像同步顯示於右上角頭像處
- WebAuthn 認證強制需填 username，登入流程/文件同步
