...
| 更新類型 | 範例 | 版本號變更 |
|---------|------|-----------|
| **重大架構變更** | 資料庫結構大改、認證系統重寫 | v0.10.0 → v1.0.0 |
| **新功能/重要改進** | 新登入方式、排班核心改進 | v0.10.0 → v0.11.0 |
| **中等功能改進** | UI/UX 優化、品牌更新、頁面重構 | v0.10.0 → v0.10.1/0.10.2 |
| **Bug修復** | 修復權限判定錯誤、資料顯示問題 | v0.10.x → v0.10.x+1 |
| **文字/樣式微調** | 按鈕文字、間距調整 | 可不增版或 v0.10.x → v0.10.x+1 |
...
**版本號更新流程 (SemVer 實務)**：
1. **決定版號**：依變更類型選擇主/次/修訂號（例：新功能→次版號；修 bug→修訂號；破壞性改動→主版號）。  
2. **同步 5 處**：`CLAUDE.md`、`README.md`、`frontend/src/pages/Settings.jsx`、`frontend/src/pages/Login.jsx`、`docs/USAGE.md`。  
3. **更新日期**：在 `CLAUDE.md`、UI 顯示處更新「最後更新」日期。  
4. **記錄內容**：於 `README.md` 版本更新記錄列出重點變更。  
5. **驗證一致**：逐檢 5 處是否一致。  
6. **提交 commit**：訊息格式「更新系統版本號至 vX.X.X」。
...
- **身份類型**: 不同護理類別具有特定班別模式
  - 麻醉專科護理師: D(日班), A(小夜班), N(大夜班), C(中班), O(休假)
